<div class="row">
        <div class="col-lg-12">
            ...
            <div class="common-history-form-box table-responsive">
                <table id="example_history" class="table table-striped table-bordered">
                    <thead>
                        <tr>
                            <th>From</th>
                            <th>To</th>
                    ...
                    <tbody>
                        @foreach ($video_history as $video_history_value)
                            <tr>
                                ...
                                @if($user_role_id == '3')
                                <td>
                                    <input type="checkbox" class="status-checkbox" data-history-id="{{ $video_history_value->videohistories_id }}" {{ $video_history_value->message_visibility == 1 ? 'checked' : '' }}>
                                </td>
                                @endif
                            </tr>                            
                        @endforeach
...
@push('pushjs')
    <script>
        ...
        $('.status-checkbox').change(function() {
            ...
            $.ajax({
                url: "{{ route('update.history.message.visibility') }}",
                type: 'POST',
                data: {
                    'historyId': historyId,
                    'isChecked': isChecked
